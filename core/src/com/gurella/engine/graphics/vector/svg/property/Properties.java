...
public class Properties {
	private static final RegExp patternSemicolon = RegExp.compile(";");
	...
	private void parseStyle(String style) {
		SplitResult styles = patternSemicolon.split(style);

		for (int i = 0; i < styles.length(); i++) {
			if (styles.get(i).length() == 0) {
				continue;
			}

			int colon = styles.get(i).indexOf(':');
			if (colon == -1) {
				continue;
			}

			String propertyName = styles.get(i).substring(0, colon).trim();
			String value = styles.get(i).substring(colon + 1).trim();
			set(propertyName, value);
		}
	}
