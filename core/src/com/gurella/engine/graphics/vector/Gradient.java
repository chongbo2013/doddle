...
public class Gradient implements Poolable, Disposable {
	...
	public static Gradient obtain() {
		return FastPools.obtainGradient();
	}
	
	public static Gradient obtain(FloatArray stops) {
		Gradient gradient = FastPools.obtainGradient();
	...
	private Texture createTexture() {
		...
		for (int i = 0; i < gradientStops; i++) {
			data.drawPixel(i * 2, 0, Color.rgba8888(getOffset(i), 0, 0, 1));
			data.drawPixel(i * 2 + 1, 0, Color.rgba8888(getRed(i), getGreen(i), getBlue(i), getAlpha(i)));
		}
	...
	public int hashCode() {
		...
		for (int i = 0; i < stops.size; i++) {
			...
			result = 31 * result + (stop != +0.0f ? NumberUtils.floatToIntBits(stop) : 0);
		}
		return result;
	}
}
