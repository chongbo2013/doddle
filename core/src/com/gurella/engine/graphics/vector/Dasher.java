...
class Dasher implements Poolable {
	...
	private Array<DashElement> dashElements = new Array<DashElement>();
	
	private PathMesh pathMesh;
	private final Array<Point> points = new Array<Point>();
	private final Array<PathComponent> dashedComponents = new Array<PathComponent>();
	...
	private final Array<Point> unusedPoints = new Array<Point>();
	...
	void dash() {
		...
		unusedPoints.removeValue(p0, true);
		...
		while (true) {
			if (currentLength + p1.length <= dashLength) {
				...
				unusedPoints.removeValue(p1, true);
	...
	private void clear() {
		...
		p1 = null;
                ...
                FastPools.resetPoints(unusedPoints);
	}
