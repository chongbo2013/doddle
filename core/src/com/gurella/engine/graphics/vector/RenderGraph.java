...
public class RenderGraph implements Poolable {
	int width;
	int height;
	GlContext glContext;
	...
	void init(GlContext glContext, FrameBuffer frameBuffer) {
		this.glContext = glContext;
		this.width = glContext.getWidth();
		this.height = glContext.getHeight();
		this.rootFrameBuffer = frameBuffer;
	}
	...
	void clear() {
                FastPools.resetCanvasLayers(allLayers);
		layersStack.clear();
		root.reset();
		currentLayer = root;
		lastSimple = null;
	...
	public static class SimpleRenderNode implements RenderNode {
		...
		static SimpleRenderNode obtain(Array<GlCall> calls) {
			SimpleRenderNode command = FastPools.obtainSimpleRenderNode();
	...
	private static abstract class CompositeRenderNode implements RenderNode {
		...
		public void reset() {
			calls.clear();
                        FastPools.resetRenderNodes(nodes);
		}
	}
	...
	static class LayerRenderNode extends CompositeRenderNode {
		...
		static LayerRenderNode obtain(RenderGraph graph, CanvasLayer layer) {
			LayerRenderNode command = FastPools.obtainLayerRenderNode();
			command.graph = graph;
			command.layer = layer;
			return command;
		}
		...
		void endLayer() {
			...
			if (effect != null) {
				effect.process(frameBuffer.getColorBufferTexture());
			}
		}
		
		GlCall getCall() {
			initVertices();
			call.getUniforms(0).imageHandle = frameBuffer.getColorBufferTexture().getTextureObjectHandle();
			return call;
		}
