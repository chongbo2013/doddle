...
public class Toast {
   ...
   private BitmapFontCache [] fontCache = new BitmapFontCache [max_toasts];
    ...
    * */
   public boolean makeText(String string, int prefered_color, int fade_mode, float x, float y, float time_to_spend) {
      ...
      font[i] = new BitmapFont(Gdx.files.internal("fonts/arial_115.fnt"), Gdx.files.internal("fonts/arial_115.png"), false);
      font[i].getData().setScale(0.5f, 0.5f);
      fontCache[i] = new BitmapFontCache(font[i]);
      fontCache[i].setColor(1, 1, 1, 1);

      // set text once so calculations work...
      fontCache[i].setText(string, x, y);

      int curWidth = (int) fontCache[i].getLayouts().get(0).width;
      int curHeight = (int) (fontCache[i].getLayouts().get(0).height * 1.25f);

      font_width[i] = curWidth + margin; // width of the string + margin
      font_height[i] = curHeight + margin; // height of the string
      float x_bounds = curWidth / 2; // we center the string
      float y_bounds = curHeight + ((curHeight + margin) * (-i)); // we offset by number of line to display in this Toast
      ...
      // Reset text after calculations...
      fontCache[i].setText(string, X[i], Y[i]);
      ...
      pix_back[i].setColor(0, 0, 0, 0.5f);
      pix_back[i].fill();
   ...
   public void toaster(SpriteBatch batch) {
      for (int j=0; j < i; j++) {
         if (time[j] != ETERNAL)
         ...
         if ((time[j] == ETERNAL ||time[j] > 0) && (fade_mode[j] == STYLE.ROUND || blend[j] > 0.15)) {
            ...
            // we draw .. finally
            batch.setColor(1, 1, 1, blend[j]);
            batch.draw(back[j], X[j]-(margin/2), Y[j]-(font_height[j])+(margin), font_width[j], font_height[j]);
            //fontCache[j].setColor(R[j], G[j], B[j], blend[j]);
            //fontCache[j].setText(string[j], X[j], Y[j]);
            fontCache[j].draw(batch);
