...
public class DrawStrokeCommand implements Command {
    ...
    /**
     * Initialize the draw line command.
     */
    public final void initLineMode(Brush activeBrush) {
        id = UUID.uuid();
        DodlesGroup group = objectManager.getNewObjectGroup();
        initLineMode(id, group, activeBrush, RulerMode.LINE, editorState.getStrokeConfig().cpy());
    }

    /**
     * Alternate initialization for the draw line command.
     */
    public final void initLineMode(String pId, BaseDodlesViewGroup group, Brush pBrush, RulerMode pRulerMode, StrokeConfig pStrokeConfig) {
        this.id = pId;        
        this.brush = pBrush;
        this.rulerMode = pRulerMode;
        this.strokeConfig = pStrokeConfig;
        
        if (addToObjects) {
            this.groupID = group.getName();
            this.phaseID = group.getActiveViewID();
        }

        initializeBrush();

        brush.setRulerMode(rulerMode);
    ...
    public void startLiveDrawing(Vector2 dodlePoint) {
        ...
        brush = brushRegistry.getActiveBrush();
    ...
    @Override
        ...
        System.out.println("Draw Stroke Command executed");
        finalizeStroke();
    ...
    public void writeConfig(Json json) {
        ...
        json.writeValue("rulerMode", rulerMode);
    ...
    public void loadConfig(JsonValue json, CommandFactory factory) {
        ...
        rulerMode = RulerMode.valueOf(json.getString("rulerMode", RulerMode.NONE.toString()));
        if (brush == null) {
